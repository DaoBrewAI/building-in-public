...
for evidence E in F.evidence:
  if E.type == "file_ref":
    ...
    b) compare current content with E.snippet:
       ...
       - none → run TIGHTENED symbol grep (see below):
          ...
          - not found → STALE
    c) if VERIFIED_*, confirm sha pin via VCS on E.path's last commit:
    ...
    a) Check ~/.claude/teams/<team>/diff-status-cache.json (15-min TTL).
       If hit: use cached status. If miss:
          check diff/PR status using your code review tool
          cache result.
    b) Read status:
       - "Merged"/"Closed" → LANDED
       - "Draft"           → DRAFT
       - "Abandoned"       → ABANDONED
       - "Open"/"Pending"  → DRAFT (not yet landed)
  ...
  elif E.type == "doc_ref" / "sql_query" / "external_url":
...
Step 2 — Language-aware definition match (only if symbol_kind+language known):
  See references/scripts/symbol_patterns.sh
...
Step 3 — Filter comments/string literals:
  bash: python3 references/scripts/symbol_in_code.py "$FILE" "$LINE" "$SYMBOL"
...
- On any tool failure (Read FileNotFound, API down): degrade gracefully (do NOT crash).
