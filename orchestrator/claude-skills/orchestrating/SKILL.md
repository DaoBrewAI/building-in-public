...
4. **Cleanup — nothing temporary outlives the mission:** after the merge succeeds, run `${CLAUDE_PLUGIN_ROOT}/scripts/orchestrator-gc.sh --hub $HUB --clean`. It automatically removes the completed mission's coordinator-planted local settings, worktrees, local `orc/*` branches, and matching `origin/orc/*` branches. It is state-gated to accepted/done/complete missions and fails closed on dirty or unverified worktrees; a refusal blocks archival until resolved. Never delete or edit shared `.claude/settings.json`, and never collect running, planned, review, blocked, or failed missions.
...
7. Set state + MISSION.md `Phase:` to `accepted`, move the mission dir into `$HUB/archive/`, delete `$HUB/.carryover-notified` (a future 65% warning may fire again), regenerate board.html, then launch any `pending` mission this unblocked (Phase 2 dependency check).
