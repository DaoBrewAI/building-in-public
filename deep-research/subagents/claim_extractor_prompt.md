...
(common: code-search, doc-search, data-agent outputs).

YOUR JOB: For each segment file, extract every concrete CLAIM that has a
specific file:line, diff/PR identifier, or sourced quote. Emit each as a
SendMessage(type="finding", source_kind="extracted", ...) following the
references/finding_schema.md schema.

EXTRACTION RULES (apply in order):
1. A claim must have at least ONE of:
   - file_ref: matches /[a-zA-Z0-9_/.-]+\.(py|cpp|hpp|h|c|cc|tsx?|jsx?|swift|kt|java|rs|go|thrift|proto|md|json|ya?ml|sh|BUILD|TARGETS)(:\d+(-\d+)?)?/
   - diff_ref: matches /\b(PR-?\d+|MR-?\d+|D\d{6,9}|#\d+)\b/
