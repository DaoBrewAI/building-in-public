...
check "Codex manifest declares the 0.4.2 feature release" bash -c \
  '[[ "$(jq -r .version "$1")" = 0.4.2 ]]' _ "$CODEX_MANIFEST"
check "Claude manifest declares the same 0.4.2 feature release" bash -c \
  '[[ "$(jq -r .version "$1")" = 0.4.2 && "$(jq -r .version "$1")" = "$(jq -r .version "$2")" ]]' \
  _ "$CLAUDE_MANIFEST" "$CODEX_MANIFEST"
check "README labels the Orchestrator 0.4.2 feature release" contains "$README" "Orchestrator 0.4.2"
