...
Keep the Claude and Codex surfaces side by side. Existing Claude skill content,
commands, hooks, scripts, and templates remain unchanged. Codex
manifests point at canonical Codex `skills/` trees while the original host
skills move to `claude-skills/`; orchestrator-specific
...
mission. It uses `workspace-write`, explicit `--add-dir` roots, and
`approval=never`, clears inherited additional writable roots, and excludes
implicit `/tmp` and `$TMPDIR` write access; it bypasses neither the sandbox nor
hook trust. Because Codex
protects linked-worktree Git metadata in this sandbox, workers produce reviewed
working-tree diffs and the launcher validates artifacts. After coordinator
inspection, a separately approved commit broker reads coordinator-owned
worktree authority outside every worker-writable root, requires the worker copy
to match, validates protected paths, and creates mission commits outside the
worker sandbox. The wrapper records its PID
immediately and captures the Codex `thread_id` from JSONL for later
`codex exec resume`. Mission state, reports, and decisions stay in the existing
`.orchestrator/` disk protocol.
...
artifact gate and linked-worktree commit broker, validate plugin manifests,
validate every skill, and smoke-test the worker wrapper's argument/error
handling without launching a paid model run.
