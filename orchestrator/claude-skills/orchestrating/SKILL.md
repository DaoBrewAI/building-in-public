...
- Plan + review stages: `claude -p --model claude-fable-5 --effort high` — planning and code review run on Fable-5 regardless of your own model, **with one automatic exception: when Fable-5 is out of quota, fall straight through to Opus and keep going.** Fable is the preference, not a hard gate; a quota wall must never park a mission waiting on a human.
...
- **Missions on one repo run in PARALLEL by default** — isolation is per-worktree, not per-repo. Two missions may share a repo as long as each owns its own worktree + `orc/<mission-slug>` branch and neither depends on the other's output. Queue as `pending` ONLY on a real dependency (see the Phase 2 check)
...
2. **Dependency check (NOT a repo-exclusivity check):** sharing a repo is fine — every mission gets its own worktree and `orc/<mission-slug>` branch, so parallel missions never write the same files. Two things still force a queue, and only these two:
   - **Same worktree or same branch.** Never launch a mission onto a worktree or branch another mission owns. Slugs are unique, so this only happens if you hand-reuse one — don't.
   - **A real output dependency.** This mission needs something the other must produce first: it builds on the other's unmerged commits, edits the same files/module in a way that would collide at merge, changes a contract the other consumes (or vice versa), or its plan/verification only makes sense after the other lands. Migrations are the classic case: two missions each adding a migration to the same repo collide on the revision chain and must be sequenced.
   When a dependency exists → leave this mission `pending`, tell the user WHICH mission it waits on and why, and launch it automatically when that one archives (or enters `failed` — Phase 6f launches it at state-change). When none exists → launch immediately, in parallel, and say so.
   Judgment call, not a lookup: if you cannot name the concrete artifact one mission needs from the other, there is no dependency — launch in parallel. When genuinely unsure whether two missions will collide at merge, ask the user rather than serializing by reflex (needless queueing wastes days) or launching blind (a merge conflict costs a rework round).
...
4. The FIRST repo (the design's center of gravity) is the **primary**; its worktree is the session cwd. Preserve any shared `<primary>/.claude/settings.json` byte-for-byte — it may enable project plugins and policies needed by the mission. Install hooks in the **primary worktree only** (hooks load from the session's cwd) by running `${CLAUDE_PLUGIN_ROOT}/scripts/install-worker-settings.sh --worktree <primary> --template ${CLAUDE_PLUGIN_ROOT}/templates/worker-settings.json --worktrees <colon-joined-absolute-worktrees> --mission-dir <mission-dir> --control-dir <control-dir> --guard-script ${CLAUDE_PLUGIN_ROOT}/scripts/worker-guard.sh --gate-script ${CLAUDE_PLUGIN_ROOT}/scripts/pipeline-gate.sh`. The installer renders `.claude/settings.local.json`, adds it to the worktree's local Git exclude, freezes its SHA-256 in coordinator-owned `worker-settings.sha256` for the launcher to verify before every Fable turn, validates the JSON and hook paths, and fails closed rather than overwrite pre-existing local settings or follow a symlinked `.claude` directory. Never edit the shared settings to install mission hooks.
...
Invoke `orchestrator:orchestrator-mediation` for the triage rules. Write `ANSWER-<n>.md` next to the BLOCKED file, append the ruling to `$HUB/DECISIONS.md` (`## D-<HOST>-<seq> (<mission-slug>, <date>) — <question> / <answer> / decided-by: orchestrator|user` — see the decision-numbering rule in Non-negotiables), write `running` back to `state` BEFORE resuming (a fast worker can write `blocked` first and be clobbered), then resume THE BACKEND THAT BLOCKED — check the last `stage:` line in `session.txt` and add `--stage exec` if it says exec:
...
4. **Cleanup — nothing temporary outlives the mission:** after the merge succeeds, first write `accepted` to state and set MISSION.md `Phase:` to `accepted`; this terminal transition makes the mission GC-eligible but does not claim cleanup success. Then run `${CLAUDE_PLUGIN_ROOT}/scripts/orchestrator-gc.sh --hub $HUB --mission <mission-slug> --clean`. It automatically removes only that completed mission's coordinator-planted local settings, worktrees, local `orc/*` branches, and matching `origin/orc/*` branches. It is state-gated to accepted/done/complete missions and fails closed on dirty or unverified worktrees; a refusal blocks archival until resolved. Never run hub-wide destructive cleanup from this acceptance path. Never delete or edit shared `.claude/settings.json`, and never collect running, planned, review, blocked, or failed missions.
...
7. Only after the mission's parent cleanup state is durably `collected`, move the mission dir into `$HUB/archive/`, delete `$HUB/.carryover-notified` (a future 65% warning may fire again), regenerate board.html, then launch any `pending` mission this unblocked (Phase 2 dependency check). If cleanup is pending, keep the accepted mission in place for mission-scoped retry and do not claim archival completion.

## Phase 6f — Failed

Entered on 2 crash deaths, 2 failed acceptance cycles, or the user saying to kill a mission. On a user-kill the session is still RUNNING: first terminate the process (find it via `pgrep -f <session-id>` as in Phase 0, then kill it) before setting `failed`. Set state + MISSION.md `Phase:` to `failed`, notify the user with what exists (branches, worktrees, the report so far), and launch any `pending` mission this unblocked (Phase 2 dependency check) — don't wait for archive. `failed` never blocks another mission (a failed mission produces no output for anything to depend on; branch names are per-slug, so the same repo is always safe). Keep worktrees/branches for salvage until the user decides; on their go-ahead, clean up as in Phase 6 step 4 and archive the mission dir.
...
- **Provision through the plugin's own scripts, never by hand-assembling their effects.** `install-worker-settings.sh` installs the hooks (into `.claude/settings.local.json`, the local layer — NOT the repo-tracked `settings.json`), `provision-preflight.sh` prepares the environment, `spawn-worker.sh` launches every stage. Read each script's `usage()` before the first call of a session rather than reproducing a command you remember: these interfaces change between plugin versions, and a remembered invocation fails in ways that look like mission problems (missing `--control-dir`, hooks written to the wrong file, settings rejected as "missing, empty, or symlinked"). If a script rejects your arguments, re-read its usage — do not patch around it by writing the files yourself.
- **Decision numbers are host-prefixed: `D-<HOST>-<seq>`.** Read `<HOST>` from the single-line plain-text file `~/.config/daobrew/orchestrator-host-tag`, trim whitespace, and use it verbatim with no derivation, abbreviation, or judgement. Only when that file is missing or empty, fall back to a short stable tag for THIS machine derived once from `hostname` — e.g. `Linhans-MacBook-Pro.local` → `LMBP`, `LY_GAMING` → `LYG`; if the hub already contains entries tagged for this machine, reuse that exact tag rather than minting a variant. `<seq>` starts at 1 and counts ONLY entries bearing this host's tag, so two machines can rule concurrently without ever colliding. Never mint, renumber, or reuse another host's tag — a remote-authored entry is read-only to you, and its number is part of how other missions cite it. Bare legacy `D-<seq>` entries predate this rule: cite them by their existing number, never renumber them opportunistically (both sides of a legacy duplicate are usually cited by archived missions, so a unilateral fix breaks citations another host owns; a deliberate one-pass cleanup by the history's owner is the only safe repair).
...
- Model assignments per stage are plugin constants — never "just this once" run execution on claude or review on codex. **The one sanctioned substitution is the quota fallback: plan/review may run on Opus when Fable-5 is exhausted** (see the quota rule below). Execution stays on codex under every circumstance.
- **Quota fallback is automatic, not a question for the user.** When a plan/review spawn dies because the model ran out of capacity — whatever form that takes: a nonzero exit, `is_error`, a refusal naming a limit, a reset time, a prompt to buy credits, or any other wording the provider invents — treat it as a quota wall, not a crash. You are a model reading a message; judge the meaning, do not pattern-match a phrase list. (`spawn-worker.sh` also exits 75 when its own matcher catches it, but that matcher is a convenience, never the thing you rely on.) On that judgment: immediately respawn the same stage with `ORC_PLAN_MODEL=claude-opus-5` prepended, note `quota-fallback: <date> <stage> fable→opus` in MISSION.md Notes, log it once in DECISIONS.md, and tell the user in one line AFTER the work is already moving — never stop and ask permission first. It is not a crash strike. The override is per-spawn and never persisted, so later stages return to Fable on their own once capacity is back. Park the mission only if the fallback model is exhausted too, and then say which quota needs topping up.
