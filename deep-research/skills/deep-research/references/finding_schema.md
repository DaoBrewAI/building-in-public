...
```json
{
  "id": "F-3",
  "agent": "researcher-2",
  "claim": "Token storage uses config service at startup",
  "evidence": [
    {
      "type": "file_ref",
    ...
    },
    {
      "type": "diff_ref",
      "diff_id": "PR-1234",
      "context": "introduced the config call in handler.py",
      "captured_at": "2026-04-21T18:32:11Z"
    },
    {
      "type": "tool_output",
      "tool": "ripgrep",
      "query": "config.get.*auth",
      "hits": 14,
      "summary": "14 hits across src/auth/, all in handler.py and config.py",
      "captured_at": "2026-04-21T18:32:11Z"
    }
  ],
  "confidence": "high",
...
- `needs_diff_landed`: true if claim asserts current production behavior; verifier will mark `[STALE]` if cited change is still in draft or abandoned.
...
### diff_ref
- `diff_id`: identifier for the PR/MR/diff (e.g., PR number, diff ID)
...
- `tool`: name (`ripgrep`, `grep`, `find`, `git`, etc.)
- `query`: exact query string
- `hits`: integer count
- `summary`: <= 200 chars
- `captured_at`: ISO-8601 UTC

### doc_ref (non-code evidence; counts toward verified_pct only if re-readable at gate time)
- `doc_id`: wiki page ID, URL, or document identifier
- `snippet`: quoted text (>= 12 chars)
- `captured_at`: ISO-8601 UTC

### sql_query
- `query_id`: query identifier (if available)
...
See `references/scripts/finding_template.sh` for a JSON skeleton emitter.
