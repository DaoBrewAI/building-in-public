...
so rendered documents show clickable superscripts + jumpable Citations section.

Also: rewrite known status tags into accessibility-safe HTML spans (HEX-pinned).
Reads:
  sys.argv[1]      draft markdown
  --no-footnotes   degrade to plain-text annotations (auto-fallback path)
Writes to stdout.
"""
...
# HEX-pinned accessibility-safe palette (avoids green for colorblind users)
TAG_COLORS = {
